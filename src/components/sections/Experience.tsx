      ...
      <div className="w-full max-w-[1720px] mx-auto px-6 sm:px-10 lg:px-14 xl:px-18 2xl:px-24 space-y-20 relative z-10">
        ...
        <div className="space-y-8">
          {EXPERIENCES.map((exp, index) => (
            <div
              key={exp.id}
              className="p-6 sm:p-8 lg:p-10 rounded-3xl border border-white/10 bg-[#090a10]/80 backdrop-blur-md hover:border-brand-blue/30 transition-all duration-300 shadow-xl space-y-6"
            >
              {/* Header Info */}
                ...
                <div className="lg:col-span-4 space-y-3">
                  ...
                  <div className="space-y-2.5">
                    {exp.impact.map((imp, idx) => (
                      <div
                        key={idx}
                        className="p-3.5 rounded-xl bg-emerald-950/20 border border-emerald-500/20 text-xs font-mono text-emerald-300 font-medium"
                      >
                        ✓ {imp}
                      </div>
                    ))}
