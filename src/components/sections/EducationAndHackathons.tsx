  ...
  return (
    <section className="relative py-28 sm:py-36 border-t border-white/10 overflow-hidden w-full">
      <div className="w-full max-w-[1720px] mx-auto px-6 sm:px-10 lg:px-14 xl:px-18 2xl:px-24 space-y-24 relative z-10">
        ...
        <SlideIn from="left" className="space-y-8">
        ...
        <FadeUp className="space-y-8">
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8" staggerDelay={0.12}>
            {HACKATHONS.filter((h) => h.isMajor).map((hack, idx) => (
              <StaggerItem key={idx}>
                <FloatCard intensity={5} className="h-full">
                  <div className="p-6 sm:p-10 rounded-3xl bg-gradient-to-br from-brand-blue/10 via-[#0a0c14] to-[#07080c] border border-brand-blue/30 shadow-xl space-y-4 relative overflow-hidden h-full">
        ...
        <FadeUp className="space-y-6">
          ...
          <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-mono text-xs" staggerDelay={0.08}>
            {EXTRACURRICULAR.map((item, idx) => (
              <StaggerItem key={idx}>
                <FloatCard>
                  <div className="p-6 rounded-2xl bg-[#090b12] border border-white/5 hover:border-brand-blue/30 transition-colors space-y-2">
        ...
        <FadeUp className="space-y-6">
          ...
          <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 font-mono text-xs" staggerDelay={0.05}>
            {CERTIFICATIONS.map((cert, idx) => (
              <StaggerItem key={idx}>
                <div className="p-4 rounded-2xl bg-[#090b12] border border-white/5 flex items-center gap-3 hover:border-white/20 transition-colors">
